...
class FakeNotificationStore {
  ...
  matchesClaim(claim) {
    return (
      this.record?.id === claim.id &&
      this.record.status === 'pending' &&
      this.record.attempts === claim.attempts &&
      this.record.lastAttemptAt?.getTime() === claim.lastAttemptAt?.getTime()
    );
  }

  async markSent(claim, messageId, now) {
    if (this.matchesClaim(claim)) {
...
test('store-backed mark helpers can mark sent and failed records', async () => {
  ...
  const claim = baseRecord({ status: 'pending' });
  const store = new FakeNotificationStore(claim);
  const sentAt = new Date('2026-06-06T00:11:00.000Z');
  await markWorshipPrepNotificationSentWithStore(store, claim, 'message-1', sentAt);
  ...
  const failedClaim = baseRecord({ status: 'pending' });
  const failedStore = new FakeNotificationStore(failedClaim);
  await markWorshipPrepNotificationFailedWithStore(failedStore, failedClaim, new Date('2026-06-06T00:12:00.000Z'));
  assert.equal(failedStore.record.status, 'failed');
});

test('store-backed mark helpers ignore stale claims after sent or newer pending attempts', async () => {
  const {
    markWorshipPrepNotificationSentWithStore,
    markWorshipPrepNotificationFailedWithStore,
  } = await loadStateModule();
  const staleClaim = baseRecord({
    status: 'pending',
    attempts: 1,
    lastAttemptAt: new Date('2026-06-06T00:00:00.000Z'),
  });
  const newerClaim = baseRecord({
    status: 'pending',
    attempts: 2,
    lastAttemptAt: new Date('2026-06-06T00:11:00.000Z'),
  });

  const sentStore = new FakeNotificationStore({ ...newerClaim, status: 'sent', messageId: 'message-2' });
  await markWorshipPrepNotificationFailedWithStore(sentStore, staleClaim, new Date('2026-06-06T00:12:00.000Z'));
  assert.equal(sentStore.record.status, 'sent');
  assert.equal(sentStore.record.messageId, 'message-2');

  const pendingStore = new FakeNotificationStore(newerClaim);
  await markWorshipPrepNotificationSentWithStore(pendingStore, staleClaim, 'message-1', new Date('2026-06-06T00:12:00.000Z'));
  assert.equal(pendingStore.record.status, 'pending');
  assert.equal(pendingStore.record.messageId, null);
});
